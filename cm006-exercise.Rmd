...
```{r}
ggplot(gapminder,aes(x=lifeExp,gdpPercap))+
  geom_point()+
  scale_y_log10()
```
...
8. Optional: git stage and cggplot(gapminder,aes(x=lifeExp,gdpPercap))+geom_point()+scale_y_log10()ommit
...
| __geometric object__  |  histpgram|
| scale                 |  linear|
| statistical transform |  none|
